...
# CONFIG
# --------------------------------------------------
st.set_page_config(layout="wide", page_title="ALPR System")
...
# --------------------------------------------------
st.title("ALPR System")
st.caption("YOLOv8 + OCR + Tracking")
...
if source_mode == "Upload Image":
    ...
    if uploaded:
        ...
        if isinstance(reads, list) and len(reads) > 0:
            ...
            latest_reads = reads[-5:]  # safe slicing

            for row in latest_reads:

                # Handle dict safely
                if isinstance(row, dict):
                    plate = row.get("plate_text") or row.get("text")
                    conf = row.get("confidence", 0)

                # Handle tuple fallback
                elif isinstance(row, (list, tuple)) and len(row) >= 2:
                    plate, conf = row[0], row[1]
                ...
                if plate:
                    if conf >= 70:
                        st.success(f"Plate: {plate} ({conf:.1f}%)")
                    else:
                        st.warning(f"Plate: {plate} (Low confidence {conf:.1f}%)")
                else:
        ...
        if isinstance(reads, list) and len(reads) > 0:
            try:
                st.dataframe(pd.DataFrame(reads), width="stretch")
            except:
                st.write(reads)
        else:
            st.info("No records")
